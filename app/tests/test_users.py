...
def test_admin_update_user_non_exists(authorized_admin_client):
    update_user = {
        "email": "updateduser@example.com"
    }
    res = authorized_admin_client.put(f"/users/100000", json=update_user)

    assert res.status_code == 404


def test_admin_update_user_with_email_taken(authorized_admin_client, test_multiple_users, test_admin_user):
    user_to_retrieve = test_multiple_users[len(test_multiple_users) - 1]
    update_user = {
